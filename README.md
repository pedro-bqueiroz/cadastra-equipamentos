...
Some functions are by far not the most efficient, but we kept them because one of the requirements was to use everything we had learned at the time. 
...
When first running the program, or to take full control, type "ICE CREAM" when logging in. You'll be able to set up users and passwords later.

In order to use the 'autocompiler' you will need to have GCC installed and in PATH. The implementation of a proper autocompiler, designed for people who have basic tech skills, will be reviewed in the future. Hopefully, they may only need to double-click the .bat file for the program to run. An .sh file is unlikely, due to the fact that a UNIX/Linux user needs to mark the file as executable using "chmod +x Cadastra-Equipamentos" after opening the program folder in Terminal.
